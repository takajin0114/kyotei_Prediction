...
# テストサーバー機能は後で実装予定
# from kyotei_predictor.static.test_server import start_test_server, stop_test_server
import time
import requests


class TestWebDisplayPhase3Fixes(unittest.TestCase):
    """Web表示機能 Phase 3 修正のテスト"""

    @classmethod
    def setUpClass(cls):
        """テストクラス全体のセットアップ"""
        print("Web表示機能 Phase 3 修正テストを開始...")
        
        # テスト用の予測データファイルを作成
        cls.test_data_dir = Path("test_outputs")
        cls.test_data_dir.mkdir(exist_ok=True)
        
        # 異なる日付形式のテストファイルを作成
        cls.create_test_prediction_files()
    ...
    def tearDownClass(cls):
        ...
        print("Web表示機能 Phase 3 修正テスト完了")

    @classmethod
    def create_test_prediction_files(cls):
        """テスト用の予測データファイルを作成"""
        # 基本のテストデータ
        base_data = {
            "prediction_date": "2025-07-15",
            "generated_at": "2025-07-15T12:00:00.000000",
            "model_info": {
                "model_name": "test_model",
                "version": "1.0.0",
                "training_date": "2025-07-14"
            },
            "execution_summary": {
                "total_venues": 1,
                "total_races": 1,
        ...
        # 異なる日付形式のファイルを作成
        test_files = [
            ("predictions_20250715.json", "2025-07-15"),  # コンパクト形式
            ("predictions_2025-07-07.json", "2025-07-07"),  # ハイフン形式
            ("predictions_2024-07-12.json", "2024-07-12"),  # 別年
        ]
        
        for filename, date in test_files:
            file_data = base_data.copy()
            file_data["prediction_date"] = date
            file_data["predictions"][0]["prediction_date"] = date
    ...
    def test_date_format_handling(self):
        ...
        for filename in available_files:
            ...
            # match.groups()は常にタプルを返すので、Noneになることはない
    ...
    def test_closest_date_finding(self):
        ...
        # JavaScriptのfindClosestDateFile関数のロジックをPythonで実装
        ...
        # テストケース
        test_cases = [
            (datetime(2025, 7, 16), 'predictions_20250715.json'),  # 最も近い
            (datetime(2025, 7, 8), 'predictions_2025-07-07.json'),  # 最も近い
            (datetime(2024, 7, 13), 'predictions_2024-07-12.json'),  # 最も近い
        ]
    ...
    def test_browser_compatibility(self):
        ...
        # テストサーバーが起動しているか確認
        try:
            response = requests.get("http://localhost:8000/kyotei_predictor/templates/predictions.html", timeout=5)
            self.assertEqual(response.status_code, 200, "テストサーバーにアクセスできません")
            print("✓ テストサーバーが正常に動作しています")
        except requests.exceptions.RequestException as e:
            self.fail(f"テストサーバーにアクセスできません: {e}")
        
        # 静的ファイルが正しく配信されているか確認
        static_files = [
            "/kyotei_predictor/static/css/predictions.css",
            "/kyotei_predictor/static/js/predictions.js"
        ]
        
        for static_file in static_files:
            try:
                response = requests.get(f"http://localhost:8000{static_file}", timeout=5)
                self.assertEqual(response.status_code, 200, f"静的ファイル {static_file} にアクセスできません")
                print(f"✓ 静的ファイル {static_file} が正常に配信されています")
            except requests.exceptions.RequestException as e:
                self.fail(f"静的ファイル {static_file} にアクセスできません: {e}")
    ...
    def test_console_error_resolution(self):
        ...
        for func_name in required_functions:
            self.assertIn(f"get{func_name}" if func_name.startswith('get') else func_name, js_content,
                         f"必要な関数 {func_name} が定義されていません")
...
def run_phase3_fixes_tests():
    """Phase 3修正テストを実行"""
    print("=" * 60)
    print("Web表示機能 Phase 3 修正テスト")
    print("=" * 60)
    
    # テストスイートを作成
    test_suite = unittest.TestLoader().loadTestsFromTestCase(TestWebDisplayPhase3Fixes)
...
if __name__ == "__main__":
    success = run_phase3_fixes_tests()
