#!/usr/bin/env python3
"""
2024年1月データを使用した段階的報酬設計モデルのハイパーパラメータ最適化スクリプト
"""
...
def objective(trial, data_dir="kyotei_predictor/data/raw/2024-01", test_mode=False):
    ...
    try:
        ...
        # テストモードの場合は短時間設定
        if test_mode:
            total_timesteps = 1000   # テスト用にさらに短縮
            n_eval_episodes = 10     # テスト用にさらに短縮
        else:
...
def optimize_graduated_reward_202401(
    n_trials=10,
    study_name="opt_202401",
    data_dir="kyotei_predictor/data/raw/2024-01",
    test_mode=True,
    resume_existing=False
):
    """2024年1月データを使用した段階的報酬設計モデルのハイパーパラメータ最適化"""
    
    print("=== 2024年1月データを使用した段階的報酬設計モデルのハイパーパラメータ最適化開始 ===")
    ...
    if os.path.exists(best_model_path):
        ...
        # 最良モデルをコピー
        import shutil
        best_model_dir = "./optuna_models/graduated_reward_best"
...
def main():
    """メイン実行関数（コマンドライン引数対応）"""
    parser = argparse.ArgumentParser(description="2024年1月データでの段階的報酬設計モデルのハイパーパラメータ最適化")
    parser.add_argument('--data-dir', type=str, default="kyotei_predictor/data/raw/2024-01", help='データディレクトリ')
    parser.add_argument('--study-name', type=str, default="graduated_reward_optimization_202401", help='Optunaスタディ名')
