...
CASE1:   prob_norm = prob / Σ(prob)（レース内で正規化）, EV = prob_norm × odds

Selection: d_hi475（4.30 <= EV < 4.75, prob >= 0.05 / prob_norm >= 0.05）
...
def _all_bets_for_race_prob_norm(
    race: dict,
    prediction_date: str,
    repo,
) -> Optional[List[BetTuple]]:
    """
    レース内で prob_norm = prob / Σ(prob)、EV = prob_norm × odds を計算し、
    d_hi475（4.30 <= EV < 4.75, prob_norm >= 0.05）を満たす bet のみ返す。
    """
    ...
    # S = Σ(prob) over all_combinations
    prob_sum = 0.0
    ...
    for c in selected:
        ...
        if od is None or float(od) <= 0:
            continue
        odds = float(od)
        prob_norm = prob / prob_sum
        ev = prob_norm * odds
        if not (EV_LO_BASELINE <= ev < EV_HI):
            continue
        if prob_norm < PROB_MIN:
            continue
        hit = _norm(comb_raw) == actual_norm
        out.append((ev, prob_norm, odds, hit))
...
def _build_day_races_baseline(
    ...
    """baseline: EV = prob × odds、d_hi475 でフィルタ済みの (max_ev, bets) を返す。"""
...
def _build_day_races_prob_norm(
    out_pred_dir: Path,
    repo,
    need_days: Set[str],
) -> Dict[str, List[Tuple[float, List[BetTuple]]]]:
    """CASE1: prob_norm = prob/Σ(prob), EV = prob_norm×odds、d_hi475 でフィルタ済み。"""
    out: Dict[str, List[Tuple[float, List[BetTuple]]]] = {}
    for day in need_days:
        path = out_pred_dir / f"predictions_baseline_{day}{STRATEGY_SUFFIX}.json"
        if not path.exists():
        ...
        races_data: List[Tuple[float, List[BetTuple]]] = []
        for race in predictions_list:
            bets = _all_bets_for_race_prob_norm(race, prediction_date, repo)
            if not bets:
                continue
            max_ev = max(ev for ev, _p, _o, _h in bets)
...
def main() -> int:
    ...
    if not db_path_resolved.exists():
        print("[EXP-0068] ERROR: DB not found: {}".format(db_path_resolved), file=sys.stderr)
        return 1
    db_path_str = str(db_path_resolved)
    print("[EXP-0068] DB found: {}".format(db_path_str))

    raw_dir = _REPO_ROOT / "kyotei_predictor" / "data" / "raw"
    if not raw_dir.is_dir():
        raw_dir = Path("kyotei_predictor/data/raw")

    output_dir = args.output_dir or _REPO_ROOT / "outputs" / "probability_normalization"
    ...
    if not out_pred_dir.exists():
        print("[EXP-0068] ERROR: Predictions dir not found: {}".format(out_pred_dir), file=sys.stderr)
    ...
    cases = [
        ("CASE0_baseline", _build_day_races_baseline(out_pred_dir, repo, need_days)),
        ("CASE1_prob_norm", _build_day_races_prob_norm(out_pred_dir, repo, need_days)),
    ]

    results_by_case: Dict[str, Dict] = {}
    for case_name, day_races in cases:
    ...
    payload = {
        ...
        "purpose": "Probability normalization: prob_norm = prob/Σ(prob), EV = prob_norm×odds. d_hi475 + switch_dd4000, n_w=36",
        "db_path_used": db_path_str,
        "n_windows": n_w,
        "predictions_source": str(out_pred_dir),
        "baseline_ev": "EV = prob × odds",
        "case1_ev": "prob_norm = prob / Σ(prob), EV = prob_norm × odds",
        "strategy": "d_hi475, switch_dd4000",
        "summary": summary_list,
        "results_by_case": results_by_case,
    }

    out_path = output_dir / "exp0068_probability_normalization.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print("Saved {}".format(out_path))

    print("\n--- EXP-0068 Probability Normalization (n_windows={}) ---".format(n_w))
