"""
EXP-0045: EV帯の頑健性確認（長期・複数分割での再現性）。

EXP-0044 で採用した variant_g（4.40≤EV<4.85）と variant_d（4.30≤EV<4.80, prob≥0.05）が、
n_w=18 の局所最適ではなく、より長い期間・複数分割でも再現するかを検証する。

評価ロジックは EXP-0044 と同一（stake=100, payout=stake*odds if hit, profit=payout-stake）。
追加で longest_losing_streak / worst_window_profit / median_window_profit / window_profit_std、
前半・後半（early half / late half）の regime 差を出力する。
"""
...
# 頑健性確認: reference_1, reference_2, variant_g, variant_d の4条件
SELECTION_VARIANTS: List[Tuple[str, float, float, Optional[float]]] = [
    ("reference_1", 4.3, 4.9, None),   # 4.3≤EV<4.9
    ("reference_2", 4.3, 4.9, 0.05),   # 4.3≤EV<4.9, prob≥0.05
    ("variant_g", 4.40, 4.85, None),   # EXP-0044 採用
...
def main() -> int:
    ...
    parser.add_argument("--n-windows", type=int, default=24, help="Number of rolling windows (24 or 30 for robustness)")
    ...
    print(f"[EXP-0045] DB found: {db_path_str}")
    ...
    if need_days and need_days.issubset(existing):
        print(f"EXP-0045: Using existing predictions in {out_pred_dir} ({len(existing)} files).")
    else:
        print(f"EXP-0045: Running rolling validation (n_windows={args.n_windows})...")
        run_rolling_validation_roi(
            db_path=db_path_str,
    ...
    out_path = output_dir / "exp0045_ev_band_robustness_verified_results.json"
    payload = {
        "experiment_id": "EXP-0045",
        "n_windows": n_w,
        "db_path_used": db_path_str,
        "stake_fixed": FIXED_STAKE,
        "evaluation_logic": "Same as EXP-0044: stake=100, payout=stake*odds if hit, profit=payout-stake",
        "selection_variants": [
    ...
    print("\n--- EXP-0045 EV band robustness (stake=100, n_windows={}) ---".format(n_w))
    print("variant      | ROI     | total_profit | max_dd   | profit/1k  | bet_count | lose_w | longest_lose | worst_w | median_w | early_half | late_half")
    print("-" * 140)
    for name, _, _, _ in SELECTION_VARIANTS:
        r = results[name]
        print(
            f"  {name:12} | {r['ROI']:6}% | {r['total_profit']:12} | {r['max_drawdown']:8} | {r['profit_per_1000_bets']:10} | {r['bet_count']:9} | {r['losing_windows']:6} | {r['longest_losing_streak']:12} | {r.get('worst_window_profit') or 0:7} | {r.get('median_window_profit') or 0:7} | {r['early_half_profit']:11} | {r['late_half_profit']:10}"
        )
