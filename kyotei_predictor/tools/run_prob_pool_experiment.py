"""
EXP-0011: 確率上位K候補プール制限型 selection（top_n_ev_prob_pool）のグリッド実験。

実験パラメータ:
  pool_k = 3, 5, 8
  top_n = 2, 3
  ev_threshold = 1.15, 1.18, 1.20
  confidence_type = None（EV のみ）または pred_prob / prob_gap

ベースライン top_n_ev (top_n=3, ev=1.18 / 1.20) と比較。結果は outputs/prob_pool_experiments/ に保存。
"""
...
def main() -> int:
    ...
    parser.add_argument("--quick", action="store_true", help="小さいグリッド（baseline + pool_k=5, top_n=3, ev=1.18 のみ）")
    args = parser.parse_args()

    raw_dir = _REPO_ROOT / "kyotei_predictor" / "data" / "raw"
    if not raw_dir.is_dir():
        raw_dir = Path("kyotei_predictor/data/raw")
    output_dir = args.output_dir or _REPO_ROOT / "outputs" / "prob_pool_experiments"
    output_dir.mkdir(parents=True, exist_ok=True)

    strategies: list = []
    # ベースライン: top_n_ev (top_n=3, ev=1.18 / 1.20)
    strategies.append(("top_n_ev_ev118", "top_n_ev", 3, 1.18))
    strategies.append(("top_n_ev_ev120", "top_n_ev", 3, 1.20))
    if args.quick:
        strategies.append(("probpool_k5_top3_ev118", "top_n_ev_prob_pool", 3, 1.18, None, 5, None))
    else:
        for pk in POOL_K_VALUES:
            for tn in TOP_N_VALUES:
                for ev in EV_VALUES:
                    name = f"probpool_k{pk}_top{tn}_ev{int(ev*100)}"
                    strategies.append((name, "top_n_ev_prob_pool", tn, ev, None, pk, None))
    ...
    for s in summaries:
        ...
        print(
            ...
            f"bets={s.get('total_selected_bets')} pool_k={s.get('pool_k')} hit_rate_rank1={s.get('hit_rate_rank1_pct')}"
        )
    ...
    out_name = "exp0011_prob_pool_results.json"
    ...
    print("\n--- Summary table ---")
    print(
        f"{'strategy_name':<28} {'roi%':>8} {'diff_base':>9} {'bets':>7} "
        f"{'races_sel':>9} {'sel_ratio':>9} {'avg_bet/race':>11} {'hit_r1%':>8}"
    )
    for r in results:
        print(
            f"{str(r.get('strategy_name') or ''):<28} "
