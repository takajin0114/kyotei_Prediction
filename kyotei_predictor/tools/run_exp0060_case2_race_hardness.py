...
CASE2 は「難しいレース」（top1_probability が低い）に強いという仮説を検証する。
CASE2_base（EV≥4.60）に top1_prob ≤ 0.35 / 0.40 / 0.45 の hardness フィルタを追加した 4 条件を比較。
同一予測・窓・switch_dd4000（baseline ref_profit から算出）。selection は d_hi475 系・rolling windows。
"""
...
def _make_filter_case2_hard(top1_max: float) -> FilterFn:
    def _f(bets: List[BetTuple], top1: Optional[float]) -> List[BetTuple]:
        if top1 is not None and top1 > top1_max:
            return []
        return _filter_bets_by_selection(bets, 4.60, EV_HI, 0.05)
    return _f
...
FILTERS: Dict[str, FilterFn] = {
    ...
    "CASE2_hard_35": _make_filter_case2_hard(0.35),
    "CASE2_hard_40": _make_filter_case2_hard(0.40),
    "CASE2_hard_45": _make_filter_case2_hard(0.45),
}
...
def _run_one_n_windows(
    ...
    """Baseline で ref_profit を算出し switch_dd4000 スケジュールを適用。4 variant のみ集計。"""
    ...
    # Baseline for ref_profit and schedule (d_hi475 相当: 4.30, EV_HI, 0.05)
...
def main() -> int:
    ...
    if not db_path_resolved.exists():
        print("[EXP-0060] ERROR: DB not found: {}".format(db_path_resolved), file=sys.stderr)
        return 1
    db_path_str = str(db_path_resolved)
    print("[EXP-0060] DB found: {}".format(db_path_str))
    ...
    if need_days and not need_days.issubset(existing):
        print("EXP-0060: Running rolling validation (n_windows={})...".format(max_n_w))
        run_rolling_validation_roi(
            db_path=db_path_str,
        ...
        print("EXP-0060: Using existing predictions in {}.".format(out_pred_dir))
    ...
    payload = {
        "experiment_id": "EXP-0060",
        "purpose": "CASE2 race hardness filter (top1_prob ≤ 0.35/0.40/0.45)",
    ...
    out_path = output_dir / "exp0060_case2_race_hardness_results.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print("Saved {}".format(out_path))

    print("\n--- EXP-0060 Summary (n_windows=36) ---")
