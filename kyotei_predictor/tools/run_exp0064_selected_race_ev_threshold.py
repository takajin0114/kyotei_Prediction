...
EXP-0063 で有効だった Selected Race EV Filter の閾値最適化。
race_selected_ev = Σ(selected_prob × odds)（selected = d_hi475 通過 bet）でレースをフィルタし、
閾値 1.02〜1.08 を 0.01 刻み等で探索する。

CASE0: baseline（フィルタなし）
CASE1: race_selected_ev ≥ 1.02
CASE2: race_selected_ev ≥ 1.03
CASE3: race_selected_ev ≥ 1.04
CASE4: race_selected_ev ≥ 1.05
CASE5: race_selected_ev ≥ 1.06
CASE6: race_selected_ev ≥ 1.08

既存戦略: d_hi475 + switch_dd4000。n_windows=36。
"""
...
THRESHOLD_CASES: List[Tuple[str, Optional[float]]] = [
    ("CASE0_baseline", None),
    ("CASE1_race_selected_ev_ge_102", 1.02),
    ("CASE2_race_selected_ev_ge_103", 1.03),
    ("CASE3_race_selected_ev_ge_104", 1.04),
    ("CASE4_race_selected_ev_ge_105", 1.05),
    ("CASE5_race_selected_ev_ge_106", 1.06),
    ("CASE6_race_selected_ev_ge_108", 1.08),
]
...
def _race_selected_ev_for_race(
    ...
    """実際に賭ける bet（d_hi475 で選ばれる bet）のみで race_selected_ev = Σ(selected_prob × odds) を計算。"""
...
def _run_one_n_windows(
    ...
    """day_races_raw を race_selected_ev でフィルタし、d_hi475 + switch_dd4000 で集計。"""
...
def main() -> int:
    ...
    if not db_path_resolved.exists():
        print("[EXP-0064] ERROR: DB not found: {}".format(db_path_resolved), file=sys.stderr)
        return 1
    db_path_str = str(db_path_resolved)
    print("[EXP-0064] DB found: {}".format(db_path_str))

    raw_dir = _REPO_ROOT / "kyotei_predictor" / "data" / "raw"
    if not raw_dir.is_dir():
        raw_dir = Path("kyotei_predictor/data/raw")

    output_dir = args.output_dir or _REPO_ROOT / "outputs" / "selected_race_ev_threshold"
    ...
    if need_days and not need_days.issubset(existing):
        print("EXP-0064: Running rolling validation (n_windows={})...".format(n_w))
        run_rolling_validation_roi(
            db_path=db_path_str,
        ...
        print("EXP-0064: Using existing predictions in {}.".format(out_pred_dir))
    ...
    payload = {
        ...
        "purpose": "Selected Race EV threshold search: race_selected_ev >= 1.02/1.03/1.04/1.05/1.06/1.08, n_w=36",
        "db_path_used": db_path_str,
        "n_windows": n_w,
        "threshold_cases": [{"name": c[0], "race_selected_ev_min": c[1]} for c in THRESHOLD_CASES],
    ...
    out_path = output_dir / "exp0064_selected_race_ev_threshold.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print("Saved {}".format(out_path))

    print("\n--- EXP-0064 Selected Race EV Threshold (n_windows={}) ---".format(n_w))
