"""
EXP-0032: EXP-0031 の正式評価のため、no_skip / skip_top10pct / skip_top20pct を
longer horizon（n_windows を 12 より長く設定可能）で比較する。

ベース: EXP-0015 + confidence_weighted（ev_gap_high=0.11, normal_unit=0.5）
比較: no_skip, skip_top10pct, skip_top20pct
出力: outputs/ev_high_skip_experiments/
"""
...
def _max_ev_for_race(race: dict) -> float:
    """レースの selected_bets に対応する EV の最大値を返す。"""
...
def main() -> int:
    ...
    parser.add_argument("--n-windows", type=int, default=18, help="Number of rolling windows (e.g. 12 or 18 for longer horizon)")
    ...
    output_dir = args.output_dir or _REPO_ROOT / "outputs" / "ev_high_skip_experiments"
    output_dir.mkdir(parents=True, exist_ok=True)
    out_pred_dir = output_dir / "rolling_roi_predictions"

    # 指定 n_windows で rolling 予測を生成（longer horizon のため当該 n_windows で再実行）
    pred_glob = list(out_pred_dir.glob("predictions_baseline_*" + STRATEGY_SUFFIX + ".json")) if out_pred_dir.exists() else []
    need_run = len(pred_glob) == 0  # 既存があっても n_windows が異なる可能性があるため、毎回実行するかは運用で調整可能
    # 本実験では指定 n_windows で一貫して評価するため、rolling を実行
    print(f"EXP-0032: Running rolling validation (n_windows={args.n_windows}) for EXP-0015...")
    run_rolling_validation_roi(
    ...
    out_path = output_dir / "exp0032_ev_high_skip_longer_horizon_results.json"
    payload = {
    ...
    print("\n--- EXP-0032 Results (n_windows={}) ---".format(args.n_windows))
