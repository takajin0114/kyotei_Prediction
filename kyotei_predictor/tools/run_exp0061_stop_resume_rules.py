...
CASE6 / CASE2 / MIX（CASE6+CASE2 50%）に、window 内で N 連敗したら残り bet を停止するルールを適用し、
max_drawdown・longest_losing_streak・total_profit のバランスを評価する。
停止は当該 window のみ。次 window で自動再開。
"""
...
# (stake, hit, odds) per bet
BetStakeHit = Tuple[float, bool, float]
...
def _apply_stop_rule(bets: List[BetStakeHit], stop_after: int) -> Tuple[float, float, int]:
    """bets を先頭から走査し、stop_after 連敗で打ち切り。total_stake, total_payout, bet_count を返す。"""
    if stop_after <= 0:
        total_stake = sum(b[0] for b in bets)
        total_payout = sum(b[0] * b[2] for b in bets if b[1])
        return (total_stake, total_payout, len(bets))
    total_stake = 0.0
    total_payout = 0.0
    consecutive_loss = 0
    count = 0
    for stake, hit, odds in bets:
        if consecutive_loss >= stop_after:
            break
        total_stake += stake
        if hit:
            ...
            consecutive_loss += 1
        count += 1
    return (total_stake, total_payout, count)
...
def _run_one_n_windows(
    ...
    """9 variants. Stop rule: N 連敗で当該 window 内の残り bet を停止。次 window で再開。"""
    ...
    # Per day: list of (stake, hit, odds) for CASE6, CASE2, MIX (CASE6+C2 50%)
    day_bets_case6: Dict[str, List[BetStakeHit]] = {}
    day_bets_case2: Dict[str, List[BetStakeHit]] = {}
    day_bets_mix: Dict[str, List[BetStakeHit]] = {}
    ...
    for day, races_data in day_races_raw.items():
        ...
        c6_list: List[BetStakeHit] = []
        c2_list: List[BetStakeHit] = []
        mix_list: List[BetStakeHit] = []
        for max_ev, bets, top1_prob in races_sub:
            c6_b = [(float(st), hit, odds) for _ev, _prob, odds, hit in FILTER_CASE6(bets, top1_prob)]
            c2_b = [(float(st), hit, odds) for _ev, _prob, odds, hit in FILTER_CASE2(bets, top1_prob)]
            for t in c6_b:
                c6_list.append(t)
            for t in c2_b:
                c2_list.append(t)
            ...
            for (s, h, o) in c2_b:
                d[(o, h)] += s * 0.5
            for (o, h), s in d.items():
                mix_list.append((s, h, o))

        day_bets_case6[day] = c6_list
        day_bets_case2[day] = c2_list
        day_bets_mix[day] = mix_list

    # Per window: concatenate bets in day order, then apply stop or not
    wi_to_days: Dict[int, List[str]] = defaultdict(list)
    for day, wi in day_to_wi.items():
        if wi in window_set and day in day_bets_case6:
            wi_to_days[wi].append(day)
    for wi in range(n_w):
        wi_to_days[wi].sort()

    def _bets_for_window(wi: int, kind: str) -> List[BetStakeHit]:
        out: List[BetStakeHit] = []
        if kind == "CASE6":
            for day in wi_to_days[wi]:
                out.extend(day_bets_case6[day])
        elif kind == "CASE2":
            for day in wi_to_days[wi]:
                out.extend(day_bets_case2[day])
        else:
            for day in wi_to_days[wi]:
                out.extend(day_bets_mix[day])
        return out
    ...
    for wi in range(n_w):
        if wi not in window_set:
            continue
        base_c6 = _bets_for_window(wi, "CASE6")
        base_c2 = _bets_for_window(wi, "CASE2")
        base_mix = _bets_for_window(wi, "MIX")

        for vname, stop_n, kind in [
            ("CASE6_base", 0, "CASE6"),
            ("CASE6_stop3", 3, "CASE6"),
            ("CASE6_stop4", 4, "CASE6"),
            ("CASE2_base", 0, "CASE2"),
            ...
            if kind == "CASE6":
                ...
                bets_wi = base_mix
            stake_wi, payout_wi, count_wi = _apply_stop_rule(bets_wi, stop_n)
            hit_wi = sum(1 for b in bets_wi[:count_wi] if b[1])
            results[vname]["window_profits"][wi] = payout_wi - stake_wi
            results[vname]["window_stake"][wi] = stake_wi
            results[vname]["window_payout"][wi] = payout_wi
            results[vname]["window_bet_count"][wi] = count_wi
            results[vname]["window_hit_count"][wi] = hit_wi
            results[vname]["total_stake"] += stake_wi
            results[vname]["total_payout"] += payout_wi
            results[vname]["bet_count"] += count_wi
            results[vname]["hit_count"] += hit_wi
    ...
    for vname in VARIANT_NAMES:
        ...
        profit_per_drawdown = round(total_profit / dd, 4) if dd > 0 else None
...
def main() -> int:
    ...
    if not db_path_resolved.exists():
        print("[EXP-0061] ERROR: DB not found: {}".format(db_path_resolved), file=sys.stderr)
        return 1
    db_path_str = str(db_path_resolved)
    print("[EXP-0061] DB found: {}".format(db_path_str))
    ...
    if need_days and not need_days.issubset(existing):
        print("EXP-0061: Running rolling validation (n_windows={})...".format(max_n_w))
        run_rolling_validation_roi(
            db_path=db_path_str,
        ...
        print("EXP-0061: Using existing predictions in {}.".format(out_pred_dir))
    ...
    payload = {
        "experiment_id": "EXP-0061",
        "purpose": "Stop/Resume rules (N consecutive losses stop in-window, resume next window)",
    ...
    out_path = output_dir / "exp0061_stop_resume_rules_results.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print("Saved {}".format(out_path))

    print("\n--- EXP-0061 Summary (n_windows=36) ---")
    if 36 in results_by_n_w:
        summary = results_by_n_w[36]["summary"]
        print("variant         | ROI     | total_profit | max_dd   | profit/1k   | bet_count | lose_w | longest_lose | total_stake | profit/dd")
        print("-" * 135)
        for s in summary:
            print("  {:13} | {:6}% | {:12} | {:8} | {:11} | {:9} | {:6} | {:12} | {:11} | {}".format(
                s["variant"],
                ...
                int(s["total_stake"]),
                s.get("profit_per_drawdown") if s.get("profit_per_drawdown") is not None else "—",
            ))
