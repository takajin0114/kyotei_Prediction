"""
EXP-0049: switch_dd 閾値感度確認。

EXP-0048 で有効だった「累積DD閾値で stake=80 に落とす」方式について、
閾値 3000 / 4000 / 5000 / 6000 / 7000 の違いで total_profit・max_drawdown・ROI・worst_window_profit の
バランスがどう変わるか確認し、実運用向け推奨閾値を決める。

選抜: d_hi475 固定（skip_top20pct + 4.30≤EV<4.75 + prob≥0.05）。
通常 stake=100、保守 stake=80。累積DD（normal 基準）が閾値以上になった window は stake=80。
"""
...
# (名前, ルール, DD閾値). ルール: "normal_only" | "conservative_only" | "switch_dd"
THRESHOLD_VARIANTS: List[Tuple[str, str, Optional[int]]] = [
    ("normal_only", "normal_only", None),
    ("conservative_only", "conservative_only", None),
    ("switch_dd3000", "switch_dd", 3000),
    ("switch_dd4000", "switch_dd", 4000),
    ("switch_dd5000", "switch_dd", 5000),
    ("switch_dd6000", "switch_dd", 6000),
    ("switch_dd7000", "switch_dd", 7000),
]
...
def main() -> int:
    ...
    print(f"[EXP-0049] DB found: {db_path_str}")
    ...
    if need_days and need_days.issubset(existing):
        print(f"EXP-0049: Using existing predictions in {out_pred_dir} ({len(existing)} files).")
    else:
        print(f"EXP-0049: Running rolling validation (n_windows={args.n_windows})...")
        run_rolling_validation_roi(
            db_path=db_path_str,
    ...
    for name, rule, dd_thr in THRESHOLD_VARIANTS:
    ...
    for name, _rule, _thr in THRESHOLD_VARIANTS:
        r = results[name]
        wp = r["window_profits"]
        ts = r["total_stake"]
        tp = r["total_payout"]
    ...
    out_path = output_dir / "exp0049_dd_threshold_sensitivity_verified_results.json"
    payload = {
        ...
        "variants": [
            {"name": n, "rule": r, "dd_threshold": t} for n, r, t in THRESHOLD_VARIANTS
        ],
