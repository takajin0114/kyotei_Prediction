...
EXP-0075 で probability cap が有効である可能性が確認されたが、
bet_count が少なく ROI が極端に高いため頑健性を検証する。
rolling window horizon (n_windows = 24 / 30 / 36) を変えて比較し、結果の再現性を確認する。
...
ref_profit は EXP-0070 と同一（4.30 ≤ EV < 4.75, prob ≥ 0.05）で算出し全 CASE・全 horizon で共通 schedule 使用。
"""
...
def main() -> int:
    ...
    if not db_path_resolved.exists():
        print("[EXP-0076] ERROR: DB not found: {}".format(db_path_resolved), file=sys.stderr)
        return 1
    db_path_str = str(db_path_resolved)
    print("[EXP-0076] DB found: {}".format(db_path_str))

    raw_dir = _REPO_ROOT / "kyotei_predictor" / "data" / "raw"
    if not raw_dir.is_dir():
        raw_dir = Path("kyotei_predictor/data/raw")

    output_dir = args.output_dir or _REPO_ROOT / "outputs" / "prob_cap_robustness"
    ...
    # Collect all days we might need (max n_windows = 36)
    n_w_max = max(N_WINDOWS_LIST)
    window_list_full = build_windows(
        min_date, max_date, TRAIN_DAYS, TEST_DAYS, STEP_DAYS, n_w_max
    )
    if len(window_list_full) < n_w_max:
        n_w_max = len(window_list_full)
    window_list_36 = list(window_list_full[:n_w_max])

    need_days = set()
    for _ts, _te, tst, tend in window_list_36:
    ...
    if need_days and not need_days.issubset(existing):
        print("[EXP-0076] Running rolling validation (n_windows={})...".format(n_w_max))
        run_rolling_validation_roi(
            db_path=db_path_str,
    ...
    for n_w in N_WINDOWS_LIST:
        ...
        # ref_profit: EXP-0070 と同一（4.30 ≤ EV < 4.75, prob ≥ 0.05）で算出し全 CASE 共通 schedule
        ...
        for case_name, ev_lo, ev_hi, prob_min, prob_max in CASES:
            res = _run_one_case(
                n_w_actual, window_list, day_races_raw, ev_lo, ev_hi, prob_min, prob_max, schedule
            )
    ...
    payload = {
        "experiment_id": "EXP-0076",
        "purpose": "probability cap robustness check: n_windows 24/30/36, CASE0-CASE3",
    ...
    out_path = output_dir / "exp0076_prob_cap_robustness.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print("Saved {}".format(out_path))

    print("\n--- EXP-0076 probability cap robustness (variant × horizon) ---")
    print("variant  | n_w  | ROI     | total_profit | max_drawdown | profit/1k   | bet_count | longest_lose | block_profit")
    print("-" * 130)
    for s in summary_list:
        bp_str = str(s["block_profit"][:4]) + "..." if len(s["block_profit"]) > 4 else str(s["block_profit"])
