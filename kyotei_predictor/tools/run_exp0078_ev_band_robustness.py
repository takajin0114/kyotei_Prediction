...
EXP-0077 で新主軸候補 4.50 ≤ EV < 4.90, 0.05 ≤ prob < 0.09 が得られた。
本実験では n_windows を 24 / 30 / 36 / 48 に拡張し、
baseline（旧主軸）と new_main（新主軸候補）の頑健性を比較する。
...
ref_profit は EXP-0070 と同一（4.30 ≤ EV < 4.75, prob ≥ 0.05）で算出し各 horizon 内で共通 schedule.
"""
...
# (case_name, ev_lo, ev_hi, prob_min, prob_max)
CASES: List[Tuple[str, float, float, float, float]] = [
    ("baseline", 4.30, 4.75, 0.05, 0.09),   # 旧主軸
    ("new_main", 4.50, 4.90, 0.05, 0.09),   # 新主軸候補（EXP-0077）
]
...
def main() -> int:
    ...
    if not db_path_resolved.exists():
        print("[EXP-0078] ERROR: DB not found: {}".format(db_path_resolved), file=sys.stderr)
        return 1
    db_path_str = str(db_path_resolved)
    print("[EXP-0078] DB found: {}".format(db_path_str))

    raw_dir = _REPO_ROOT / "kyotei_predictor" / "data" / "raw"
    if not raw_dir.is_dir():
        raw_dir = Path("kyotei_predictor/data/raw")

    output_dir = args.output_dir or _REPO_ROOT / "outputs" / "ev_band_robustness"
    ...
    if need_days and not need_days.issubset(existing):
        print("[EXP-0078] Running rolling validation (n_windows={})...".format(n_w_max))
        run_rolling_validation_roi(
            db_path=db_path_str,
        ...
        print("[EXP-0078] Using existing predictions.")
    ...
    payload = {
        "experiment_id": "EXP-0078",
        "purpose": "EV band robustness: baseline vs new_main (4.50-4.90), n_w=24/30/36/48",
    ...
    out_path = output_dir / "exp0078_ev_band_robustness.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print("Saved {}".format(out_path))

    print("\n--- EXP-0078 EV band robustness (variant × horizon) ---")
    ...
    for s in summary_list:
        ...
        print(" {:9} | {:4} | {:6}% | {:12} | {:12} | {:11} | {:9} | {:12} | {}".format(
            s["variant"],
            s["n_windows"],
