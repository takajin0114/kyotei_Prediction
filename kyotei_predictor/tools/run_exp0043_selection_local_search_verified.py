"""
EXP-0043: selection 条件の局所探索（baseline_c / baseline_b 周辺・厳密評価）。

EXP-0042 で最もマシだった条件は baseline_c（4≤EV<5）と baseline_b（3≤EV<5+prob≥0.05）。
本実験では EV 帯と prob 閾値を微調整し、厳密評価のまま赤字幅を縮小できるかを検証する。

評価ロジックは EXP-0042 と同一:
- stake = 100（固定）
...
# 局所探索: baseline_c (4≤EV<5) / baseline_b (3≤EV<5+prob≥0.05) 周辺
# (name, ev_lo, ev_hi, prob_min). prob_min None = 制約なし
SELECTION_VARIANTS: List[Tuple[str, float, float, Optional[float]]] = [
    ("reference_1", 3.0, 5.0, 0.05),   # baseline_b 相当
    ("reference_2", 4.0, 5.0, None),   # baseline_c 相当
    ("variant_a", 4.0, 5.0, None),      # baseline_c 再掲
    ("variant_b", 4.0, 5.0, 0.03),
    ("variant_c", 4.0, 5.0, 0.05),
    ("variant_d", 4.0, 4.8, None),
    ("variant_e", 4.0, 4.8, 0.03),
    ("variant_f", 4.0, 4.8, 0.05),
    ("variant_g", 4.2, 5.0, None),
    ("variant_h", 4.2, 5.0, 0.03),
    ("variant_i", 4.2, 5.0, 0.05),
    ("variant_j", 4.3, 4.9, None),
    ("variant_k", 4.3, 4.9, 0.03),
    ("variant_l", 4.3, 4.9, 0.05),
]
...
def main() -> int:
    ...
    print(f"[EXP-0043] DB found: {db_path_str}")
    ...
    if need_days and need_days.issubset(existing):
        print(f"EXP-0043: Using existing predictions in {out_pred_dir} ({len(existing)} files).")
    else:
        print(f"EXP-0043: Running rolling validation (n_windows={args.n_windows})...")
        run_rolling_validation_roi(
            db_path=db_path_str,
    ...
    out_path = output_dir / "exp0043_selection_local_search_verified_results.json"
    payload = {
        "experiment_id": "EXP-0043",
        "db_path_used": db_path_str,
        "stake_fixed": FIXED_STAKE,
        "evaluation_logic": "Same as EXP-0042: stake=100, payout=stake*odds if hit, profit=payout-stake",
        "selection_variants": [
    ...
    print("\n--- EXP-0043 Selection local search (stake=100, n_windows={}) ---".format(args.n_windows))
