"""
EXP-0022: venue 別 EV 閾値戦略（top_n_ev_gap_venue_filter）の評価。

top_n_ev_gap_filter に会場別 ev_threshold（venue_ev_config）を適用し、
...
# デフォルト venue_ev_config（会場別 EV 閾値）
# 未指定会場は EV_THRESHOLD を使用
DEFAULT_VENUE_EV_CONFIG = {
    "TODA": 1.23,
    "SUMINOE": 1.17,
}
...
def main() -> int:
    ...
    output_dir = args.output_dir or _REPO_ROOT / "outputs" / "venue_filter_experiments"
    output_dir.mkdir(parents=True, exist_ok=True)

    # 1) ベースライン: top_n_ev_gap_filter（EXP-0015 同一条件）
    # 2) top_n_ev_gap_venue_filter: デフォルト ev + venue_ev_config
    strategies = [
        (
        ...
        (
            "venuefilter_ev120_evgap0x07",
            "top_n_ev_gap_venue_filter",
            TOP_N,
            EV_THRESHOLD,
            None,
            None,
    ...
    for s in summaries:
        ...
        row = {
            ...
            "venue_ev_config": s.get("venue_ev_config"),
    ...
    out_path = output_dir / "exp0022_venue_filter_results.json"
    payload = {
        ...
        "config": {
            ...
            "venue_ev_config": DEFAULT_VENUE_EV_CONFIG,
        },
