"""
EXP-0030: EXP-0015（top_n_ev_gap_filter, top_n=3, ev=1.20, ev_gap=0.07）をベースラインとして、
fixed と confidence_weighted sizing（ev_gap_high=0.11, normal_unit=0.5）を正式比較する。
...
# EXP-0015 のみ: (name, strategy, top_n, ev_threshold, ..., ev_gap_threshold)
...
def main() -> int:
    ...
    print(f"EXP-0030: Running rolling validation (n_windows={args.n_windows}) for EXP-0015 only...")
    ...
    for strat_name in ["exp0015"]:
        for sname in ["fixed", "confidence_weighted"]:
            r = results[strat_name][sname].copy()
            r["strategy_id"] = strat_name
            r["strategy_name"] = f"{strat_name}_{sname}"
            results_list.append(r)

    out_path = output_dir / "exp0030_confidence_weighted_formal_baseline_results.json"
    payload = {
        "experiment_id": "EXP-0030",
        "baseline_condition": "EXP-0015: top_n_ev_gap_filter, top_n=3, ev=1.20, ev_gap=0.07",
        "n_windows": args.n_windows,
        "seed": args.seed,
        "strategies": [
    ...
    print("\n--- EXP-0030 Results (sizing_name | ROI | total_profit | max_drawdown | profit_per_1000_bets | bet_count) ---")
    for strat_name in ["exp0015"]:
        for sname in ["fixed", "confidence_weighted"]:
            r = results[strat_name][sname]
            print(
                f"  {sname} | ROI={r.get('overall_roi_selected')}% | "
