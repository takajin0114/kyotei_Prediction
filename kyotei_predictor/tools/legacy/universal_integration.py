...
Google Colab、ローカル環境、その他のクラウド環境で実行可能な
汎用的な統合スクリプト
"""
...
class UniversalIntegration:
    ...
    def setup_environment(self):
        """環境のセットアップ"""
        print(f"🔧 {self.environment.upper()}環境をセットアップ中...")
        print(f"   プラットフォーム: {self.env_info['platform']}")
        print(f"   Python: {self.env_info['python_version'].split()[0]}")
        print(f"   作業ディレクトリ: {self.env_info['working_directory']}")
    ...
    def import_environment_modules(self):
        ...
        if self.environment == "colab":
            try:
                ...
                print("⚠️ Google Colab files モジュールのインポートに失敗しました")
        
        elif self.environment == "jupyter":
            try:
                from IPython.display import display, HTML
                self.display = display
                print("✅ Jupyter display モジュールをインポートしました")
            except ImportError:
                print("⚠️ Jupyter display モジュールのインポートに失敗しました")
    
    def create_universal_settings(self):
        """汎用設定ファイルを作成"""
        print("⚙️ 汎用設定ファイルを作成中...")
        ...
        universal_settings = {
            "project_name": "kyotei_Prediction_Universal",
        ...
        with open("universal_settings.json", "w", encoding="utf-8") as f:
            json.dump(universal_settings, f, ensure_ascii=False, indent=2)
        
        print("✅ 汎用設定ファイルを作成しました: universal_settings.json")
    ...
    def create_universal_prediction_engine(self):
        """汎用予測エンジンを作成"""
        print("🤖 汎用予測エンジンを作成中...")
...
class UniversalPredictionEngine:
    ...
    def save_results(self, results: Dict, filename: str = "prediction_results.json"):
        """結果を保存"""
        try:
...
'''
        ...
        with open("universal_prediction_engine.py", "w", encoding="utf-8") as f:
            f.write(prediction_engine_code)
        
        print("✅ 汎用予測エンジンを作成しました: universal_prediction_engine.py")
    ...
    def run_demo(self):
        ...
        # 設定ファイルを作成
        settings = self.create_universal_settings()
        
        # サンプルデータを作成
        race_data, odds_data = self.create_sample_data()
        
        # 予測エンジンを作成
        self.create_universal_prediction_engine()
        ...
        try:
            ...
            # 結果を保存
            all_results['trifecta_probabilities'] = trifecta_probs
            engine.save_results(all_results, "universal_prediction_results.json")
    ...
    def upload_files(self):
        """ファイルアップロード機能（環境依存）"""
        if self.environment == "colab" and self.files:
            print("📤 ファイルアップロード機能:")
            ...
            print("📁 ファイルアップロード機能:")
            print("この環境では手動でファイルを配置してください:")
            print("- kyotei_predictor/data/raw/ (レースデータ)")
            print("- kyotei_predictor/data/sample/ (サンプルデータ)")
            print("- optuna_results/ (最適化結果)")
    ...
    def create_environment_specific_scripts(self):
        """環境固有のスクリプトを作成"""
        print("🔧 環境固有のスクリプトを作成中...")
...
'''
            ...
            with open("colab_specific.py", "w", encoding="utf-8") as f:
                f.write(colab_script)
            print("✅ Colab用スクリプトを作成しました: colab_specific.py")
...
def display_results(results):
    """結果をJupyterで表示"""
    html = f"<h3>予測結果</h3><pre>{json.dumps(results, indent=2, ensure_ascii=False)}</pre>"
    display(HTML(html))
'''
            
            with open("jupyter_specific.py", "w", encoding="utf-8") as f:
                f.write(jupyter_script)
            print("✅ Jupyter用スクリプトを作成しました: jupyter_specific.py")
...
def open_results():
    """結果ファイルをブラウザで開く"""
    if os.path.exists("universal_prediction_results.json"):
        print("📁 結果ファイルを確認してください: universal_prediction_results.json")
    else:
...
def run_web_server():
    ...
    @app.route('/')
    def index():
        return "Kyotei Prediction - Universal Edition"
    
    app.run(host='localhost', port=12000, debug=True)
'''
            
            with open("local_specific.py", "w", encoding="utf-8") as f:
                f.write(local_script)
            print("✅ ローカル用スクリプトを作成しました: local_specific.py")

def main():
    """メイン処理"""
    print("🚀 汎用統合スクリプトを開始します")
    
    # 環境を自動検出
    environment = EnvironmentDetector.detect_environment()
    print(f"🔍 検出された環境: {environment}")
    
    integration = UniversalIntegration(environment)
    
    # パッケージのインストール
    integration.install_requirements()
    
    # 環境固有のスクリプトを作成
    integration.create_environment_specific_scripts()
    ...
    print("\n📋 利用可能な機能:")
    print("1. 予測エンジンの実行")
    print("2. ファイルのアップロード/配置")
    print("3. 結果のダウンロード/確認")
    print("4. 分析スクリプトの実行")
    
    print(f"\n💡 {environment.upper()}環境での次のステップ:")
    ...
    print(f"\n✅ {environment.upper()}環境でのセットアップが完了しました！")
