...
### EV ベッティング戦略の確定（EV sweep・rolling 比較後）

- **実施内容**: EV threshold sweep（1.02〜1.20）と rolling EV 比較（fixed vs kelly_half）を実行。結果は **logs/ev_threshold_sweep.json**, **logs/rolling_ev_strategy_compare.json**。
- **主戦略（確定）**: B案 + **calibration=sigmoid** + **top_n=5** + **EV>=1.15** + **bet sizing=fixed**。閾値 1.15 はスイープで相対的に損失最小、fixed は rolling で max_drawdown が kelly_half より小さいため採用。詳細は **docs/EV_BETTING_STRATEGY.md**。

### 次候補（キャリブレーションスプリント後）

1. **本番推奨**: 上記主戦略（EV 1.15 + fixed）を config / CLI デフォルトで利用。
2. **キャリブレーションの微調整**: sigmoid と isotonic の併用や、閾値の細かい比較。
