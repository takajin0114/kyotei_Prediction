...
6. **レース前情報**: 状態ベクトルは出走表＋レース情報で生成し、オッズは状態に含めない。`fetch_pre_race_data` で出走表と直前情報を統合取得し、予測ツール（`prediction_tool`）の本番取得で利用している。`run_complete_prediction(fetch_data=True)` はオッズ取得失敗時でも予測を継続する（期待値は暫定値）。
