...
実験を実行し leaderboard を更新したら、**chat_context.md** を再生成してから commit / push する。
...
3. 以下を実行して **chat_context.md** を再生成する（Leaderboard Summary・Latest Experiment を自動反映）。
   ```bash
...
- `experiments/logs/EXP-XXXX_*.md` を作成したあと、上記の **generate_chat_context** を実行すると、chat_context の「Latest Experiment」がその EXP に更新される。
...
1. Cursor で作業する
2. `bash scripts/ai_dev_cycle.sh` を実行するか、実験後は `python3 scripts/generate_chat_context.py` で chat_context を再生成する
...
| **chat_context.md** | **ChatGPT レビュー用: Project Goal / Current Production Strategy / Best Historical Result / Latest Experiment / Leaderboard Summary / Current Findings / Open Questions / Next Experiments。実験後は `python3 scripts/generate_chat_context.py` で再生成。** |
