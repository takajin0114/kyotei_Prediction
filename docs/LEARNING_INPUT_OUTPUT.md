...
KyoteiEnvManager → 1レースごとに KyoteiEnv
  - 状態: build_race_state_vector(race_data, None) → get_state_dim() 次元
...
| 種別 | 形式・場所 | 必須項目 | 用途 |
|------|------------|----------|------|
| **レースデータ** | JSON ファイル `race_data_YYYY-MM-DD_会場名_RN.json`（または DB の races テーブル） | 下記参照 | 状態ベクトル・正解着順 |
| **オッズデータ** | JSON ファイル `odds_data_YYYY-MM-DD_会場名_RN.json`（または DB の odds テーブル） | 下記参照 | 報酬計算（払戻） |

- **現状（ファイル）**: `data_dir` 配下（デフォルト `kyotei_predictor/data/raw`）。サブディレクトリ（例: `YYYY-MM/`）も検索される。
- **将来（DB）**: SQLite でレース・オッズを保管する方針。データ数増加と学習の条件指定を考慮し、[DATA_STORAGE_AND_DB.md](DATA_STORAGE_AND_DB.md) で方針・スキーマ・移行手順を整理している。実装後は `--data-source db` 等で DB を指定して学習可能にする予定。
- **ペア**: 同じ日付・会場・レース番号の `race_data_*` と `odds_data_*` が**両方ある**レースだけがエピソードに使われる。片方だけだとスキップされる（DB の場合は JOIN でペアのみ取得）。
- **フィルタ**: `--year-month 2024-05` でファイル名の日付が `2024-05` のものだけに絞れる（DB の場合は SQL の WHERE で同様に絞り込み）。
...
| キー | 用途 |
|------|------|
| odds_data | 3連単 120 通りの betting_numbers と ratio。報酬計算（払戻）に使用 |

### 2.2 状態ベクトル（観測空間）

- **次元**: `get_state_dim()`（`48 + 会場数 + 3`。現行 `venue_mapping` では 75 次元）
- **構成**（`pipelines/state_vector.py` の `build_race_state_vector`）:
  - **艇特徴**: 6艇 × 8 = 48次元（艇番正規化・級別圧縮・全国勝率・当地勝率・艇連対率・艇3連対率・モーター連対率・モーター3連対率）
  - **レース特徴**: 会場 one-hot + レース番号正規化 + 周回 + 進入固定（会場数 + 3次元）
  - **オッズ特徴**: **含めない**（オッズは状態に入れない）
- `KyoteiEnv` の `observation_space` も `shape=(get_state_dim(),)` で状態定義と一致している。
...
- **学習と予測の入力が一致している**  
  学習時の「状態」は「出走表＋レース情報」から作っており、予測時も同じ定義で状態を組む。結果の `race_records` は報酬計算専用で、状態には入れていない。
...
- **オッズのタイミング**  
  状態はオッズ非依存なので、理論上は締切前でも確率推定は可能。  
  `run_complete_prediction(fetch_data=True)` はオッズ取得に失敗しても予測を継続する。オッズ未取得レースは期待値が暫定値になるため、運用時は `odds_available` を確認するとよい。

- **直前情報がまだ状態に入っていない**  
  出走表＋レース情報で動作している一方、スタート展示・周回展示・天候（直前情報）は取得だけして状態ベクトルには未使用。**レース前に使える情報を増やす**という意味では、これらを状態に組み込むと方向性がさらに揃う（次元拡張・再学習が必要）。

- **学習データは「過去の結果付き」が前提**  
  教師信号は「実際の着順」なので、学習には必ず race_records が要る。そのため「未来のレース」は結果がないので予測専用。インとアウトの役割分担（学習＝結果あり／予測＝結果なし）ははっきりしている。

- **状態次元の揃え方**  
  環境側と予測側で別ロジックを持つと次元不一致が起きやすい。現状は `state_vector.py` の共通関数に統一されており、会場順序・特徴量定義を1箇所で管理できる。

### 5.3 まとめ

- **方向性**: 「レース前に得られる情報（出走表＋レース情報）を入力に、3連単の確率分布を出力し、オッズは回収率計算に使う」というインとアウトの関係は筋が通っている。  
- **強化するなら**: 直前情報を状態に含める、オッズ未取得時の期待値表示を改善する、状態定義の共通化を継続維持する、の3点を意識するとよい。  
