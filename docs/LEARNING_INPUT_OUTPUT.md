...
- **次元**: `get_state_dim()`（`48 + 会場数 + 3`。現行 `venue_mapping` では 73 次元）
...
- **オッズのタイミング**  
  ...
  ただし現行の `run_complete_prediction(fetch_data=True)` はオッズ取得に失敗すると処理を中断するため、締切前運用に合わせる場合はフロー側の条件緩和が必要。
...
- **強化するなら**: 直前情報を状態に含める、オッズ取得失敗時の運用分岐を明確にする、状態定義の共通化を継続維持する、の3点を意識するとよい。  
