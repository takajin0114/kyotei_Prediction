...
| - | EXP-0072 | xgboost | sigmoid | extended_features | race difficulty filter（厳密評価） | CASE2 selection + top1_prob/entropy/prob_gap フィルタ 6 条件, n_w=36 | CASE0（filter なし）が最良（ROI 8.29%, profit 4,524, max_dd 9,266）。全フィルタで baseline 未達。reject。log: experiments/logs/EXP-0072_race_difficulty_filter.md。結果: outputs/race_difficulty/exp0072_race_difficulty.json（gitignoreのため未コミット）。 |
