...
| - | EXP-0021 | xgboost | sigmoid | extended_features | top_n_ev_gap_filter | top_n=2,3,4 × ev=1.19,1.20,1.21 × ev_gap=0.06,0.07,0.08 (n_w=12) | （結果確定次第で記載） | top_n を含む局所探索。実験実行中。結果確定次第で ROI/Notes を更新。log: experiments/logs/EXP-0021_ev_gap_topn_local_search.md。 |
...
- **EXP-0021**: top_n × ev × ev_gap 局所探索（top_n=2,3,4 × ev=1.19,1.20,1.21 × ev_gap=0.06,0.07,0.08）。**結果**: 実験実行中。結果確定次第で本 Notes と log の Results を更新。tool: `python3 -m kyotei_predictor.tools.run_ev_gap_topn_local_search_experiment`。log: experiments/logs/EXP-0021_ev_gap_topn_local_search.md。結果: outputs/ev_gap_experiments/exp0021_ev_gap_topn_local_search_results.json（完了後に生成）。
