...
| - | EXP-0050 | xgboost | sigmoid | extended_features | switch_dd4000 長期頑健性（厳密評価） | normal_only/switch_dd4000/5000, n_w=24/30/36 | 30/36 でも switch_dd4000 が profit・DD で優位維持。実運用標準候補へ格上げ。log: experiments/logs/EXP-0050_switch_dd4000_robustness_verified.md。 |
| - | EXP-0051 | xgboost | sigmoid | extended_features | switch_dd4000 期間安定性・regime（厳密評価） | normal_only/switch_dd4000/conservative, n_w=36, 6block | 後半で悪化傾向。注意条件付き標準採用。log: experiments/logs/EXP-0051_switch_dd4000_regime_analysis.md。 |
| - | EXP-0052 | xgboost | sigmoid | extended_features | Seasonality/Regime 分析（厳密評価） | 月別・block・累積曲線・switch発動・regime特徴, n_w=36 | 月別・block強弱・負け期間はhit_rate高め・bet数少なめ。hold。log: experiments/logs/EXP-0052_seasonality_analysis.md。 |
| - | EXP-0053 | xgboost | sigmoid | extended_features | Payout/Odds Regime 分析（厳密評価） | 的中時オッズ・払戻・profit_per_hit の block/月別, n_w=36 | 負けblockはhit_odds低・payout_per_hit約半額。利益悪化は高配当不足で説明。hold。log: experiments/logs/EXP-0053_payout_regime_analysis.md。 |
...
| - | EXP-0062 | xgboost | sigmoid | extended_features | Race EV Filter（厳密評価） | race_ev≥1.00/1.02/1.05/1.10 + d_hi475+switch_dd4000, n_w=12 | n_w=12で全CASE同一（対象レースがrace_ev≥1.10）。log: experiments/logs/EXP-0062_race_ev_filter.md。 |
| - | EXP-0063 | xgboost | sigmoid | extended_features | Selected Race EV Filter（厳密評価） | race_selected_ev≥1.05/1.10/1.15/1.20 + d_hi475+switch_dd4000, n_w=12 | 選択betのみでEV合計。CASE1〜4でROI 11.31%、profit 2942、max_dd 6178。adopt。log: experiments/logs/EXP-0063_selected_race_ev_filter.md。 |
| - | EXP-0064 | xgboost | sigmoid | extended_features | Selected Race EV threshold search（厳密評価） | race_selected_ev≥1.02/1.03/1.04/1.05/1.06/1.08, n_w=36 | n_w=36ではbaseline最良（ROI 0.53%、profit 484）。フィルタは赤字。horizon依存。log: experiments/logs/EXP-0064_selected_race_ev_threshold.md。 |
| - | EXP-0065 | xgboost | none/sigmoid/isotonic | extended_features | Calibration comparison（厳密評価） | d_hi475+switch_dd4000, n_w=36 | sigmoid のみ黒字（ROI 0.53%、profit 484）。none -11.71%、isotonic -22.22%。sigmoid 維持 adopt。log: experiments/logs/EXP-0065_calibration_comparison.md。 |
| - | EXP-0066 | xgboost | sigmoid | extended_features | Prediction diagnostics（分析） | calibration, Brier, log loss, EV bucket ROI, n_w=36 | Brier 0.955, log loss 5.01。EV 1.5-2.0 が相対マシ -7.77%、1.2-1.5 が最悪 -29.23%。log: experiments/logs/EXP-0066_prediction_diagnostics.md。 |
| - | EXP-0067 | xgboost | sigmoid | extended_features | EV shrinkage（厳密評価） | EV_adj=EV^alpha (0.5/0.7/0.8/0.9), d_hi475+switch_dd4000, n_w=36 | 全CASE同一（ROI -3.5%、bet 943）。EV帯が狭く shrinkage で差が出ず。log: experiments/logs/EXP-0067_ev_shrinkage.md。 |
| - | EXP-0068 | xgboost | sigmoid | extended_features | Probability normalization（厳密評価） | prob_norm=prob/Σ(prob), EV=prob_norm×odds, d_hi475+switch_dd4000, n_w=36 | baseline と CASE1 同一（ROI -3.5%、bet 943）。レース内確率和≈1のため差なし。log: experiments/logs/EXP-0068_probability_normalization.md。 |
| - | EXP-0069 | xgboost | sigmoid | extended_features | EV percentile strategy（厳密評価） | d_hi475 vs top 0.5/1/2/5% EV, switch_dd4000, n_w=36 | baseline が最良（-3.5%、bet 943）。percentile は bet 増・ROI -14.31%。reject。log: experiments/logs/EXP-0069_ev_percentile.md。 |
| - | EXP-0070 | xgboost | sigmoid | extended_features | d_hi475 local search（厳密評価） | EV帯・prob_min 局所探索（CASE0〜7）, switch_dd4000, n_w=36 | 最良 CASE2（4.50≤EV<4.75, prob≥0.05）: ROI 11.12%, profit 5,772, max_dd 8,838。adopt。log: experiments/logs/EXP-0070_d_hi475_local_search.md。 |
| - | EXP-0071 | xgboost | sigmoid | extended_features | CASE2 robustness validation（厳密評価） | CASE0/CASE2/CASE3, n_w=24/30/36, switch_dd4000 | robustness validation。CASE2 は全 horizon で ROI・max_dd・profit/1k 優位。n_w=24 では total_profit は CASE0/CASE3 に及ばず。条件付き採用。log: experiments/logs/EXP-0071_case2_robustness_check.md。結果: outputs/case2_robustness/exp0071_case2_robustness.json（gitignoreのため未コミット）。 |
| - | EXP-0072 | xgboost | sigmoid | extended_features | race difficulty filter（厳密評価） | CASE2 selection + 難易度フィルタ 6 条件, n_w=36 | 初回は ref_profit 差分で baseline ずれ。修正後は EXP-0072b で再評価。log: experiments/logs/EXP-0072_race_difficulty_filter.md。 |
| - | EXP-0072b | xgboost | sigmoid | extended_features | race difficulty filter recheck（厳密評価） | ref_profit を EXP-0070 一致（CASE0 4.30~4.75）に修正後、同条件で再実行, n_w=36 | CASE0（baseline）= EXP-0070 CASE2 一致（ROI 11.12%, profit 5,772, max_dd 8,838）。全 difficulty フィルタで baseline 未達。reject。log: experiments/logs/EXP-0072b_race_difficulty_recheck.md。結果: outputs/race_difficulty/exp0072b_race_difficulty.json（gitignoreのため未コミット）。 |
| - | EXP-0075 | xgboost | sigmoid | extended_features | probability cap（厳密評価） | 4.50≤EV<4.75, 0.05≤prob<0.12/0.10/0.09/0.08, switch_dd4000, n_w=36 | 最良 CASE3（prob<0.09）: ROI 187.63%, profit 15,010, max_dd 3,980。prob 上限で全 CASE が baseline を大幅上回り adopt。log: experiments/logs/EXP-0075_prob_cap_experiment.md。結果: outputs/prob_cap/exp0075_prob_cap.json（gitignoreのため未コミット）。 |
| - | EXP-0076 | xgboost | sigmoid | extended_features | probability cap robustness（厳密評価） | CASE0〜CASE3（prob cap）, n_w=24/30/36, switch_dd4000 | 全 horizon で cap CASE が baseline を上回る。CASE3 は n_w=24: 319.89%, 30: 223.17%, 36: 187.63%。頑健性確認 adopt。log: experiments/logs/EXP-0076_prob_cap_robustness.md。結果: outputs/prob_cap_robustness/exp0076_prob_cap_robustness.json（gitignoreのため未コミット）。 |
| - | EXP-0077 | xgboost | sigmoid | extended_features | EV×prob matrix（厳密評価） | EV帯6×prob帯6の36条件, baseline 4.30≤EV<4.75, 0.05≤prob<0.09, n_w=36 | 最良 ev450_490_prob005_009: profit 17,338, ROI 159.36%, max_dd 5,400, bet 124。ev430_460 は全条件で -100% reject。adopt。log: experiments/logs/EXP-0077_ev_prob_matrix.md。結果: outputs/ev_prob_matrix/exp0077_ev_prob_matrix.json（gitignoreのため未コミット）。 |
| - | EXP-0078 | xgboost | sigmoid | extended_features | EV band robustness（厳密評価） | baseline 4.30-4.75 vs new_main 4.50-4.90, 0.05≤prob<0.09, n_w=24/30/36/48 | 全 horizon で new_main が baseline を上回る。n_w=48 で new_main profit 19,858, ROI 155.14%。頑健性確認 adopt。log: experiments/logs/EXP-0078_ev_band_robustness.md。結果: outputs/ev_band_robustness/exp0078_ev_band_robustness.json（gitignoreのため未コミット）。 |
| - | EXP-0079 | xgboost | sigmoid | extended_features | schedule/ref_profit 再最適化（厳密評価） | new_main ベット固定、ref_profit 旧基準 vs 新基準＋dd3000/4000/5000, n_w=24/30/36/48 | 全 horizon で CASE0（旧 ref）が total_profit・ROI 最大。new_ref は max_dd 改善するが profit 減。ref は旧基準維持 hold。log: experiments/logs/EXP-0079_schedule_refprofit_reopt.md。結果: outputs/schedule_refprofit_reopt/exp0079_schedule_refprofit_reopt.json（gitignoreのため未コミット）。 |
...
- **EXP-0050**: switch_dd4000 長期頑健性（normal_only / conservative_only / switch_dd4000 / switch_dd5000 を n_w=24/30/36 で比較）。**結果**: 24→30→36 で switch_dd4000 が常に profit・ROI 1位。30 で profit 4,796・max_dd 10,702、36 で唯一黒字 484・max_dd 15,886。**judgment**: 実運用標準候補へ格上げ（adopt）。tool: `python3 -m kyotei_predictor.tools.run_exp0050_switch_dd4000_robustness_verified`。log: experiments/logs/EXP-0050_switch_dd4000_robustness_verified.md。結果: outputs/selection_verified/exp0050_switch_dd4000_robustness_verified_results.json。
- **EXP-0051**: switch_dd4000 期間安定性・regime 分析（n_w=36、6ブロック）。**結果**: 全ブロックで switch が normal 以上。後半（7月後半〜10月）で成績悪化。switch 発動 22/36 窓。**judgment**: 注意条件付き標準採用（adopt with caveats）。tool: `python3 -m kyotei_predictor.tools.run_exp0051_switch_dd4000_regime_analysis`。log: experiments/logs/EXP-0051_switch_dd4000_regime_analysis.md。結果: outputs/selection_verified/exp0051_switch_dd4000_regime_analysis_results.json。
- **EXP-0052**: Seasonality/Regime 分析（月別・block・累積曲線・switch発動・regime特徴、n_w=36）。**結果**: 月別で08・09が悪化。負けブロックは bet 数少なめ・hit_rate 高め・EV 同程度。**judgment**: 分析実験として知見整理（hold）。tool: `python3 -m kyotei_predictor.tools.run_exp0052_seasonality_analysis`。log: experiments/logs/EXP-0052_seasonality_analysis.md。結果: outputs/selection_verified/exp0052_seasonality_analysis_results.json。
- **EXP-0053**: Payout/Odds Regime 分析（的中時オッズ・払戻・profit_per_hit の block/月別、n_w=36）。**結果**: 勝ちblockは avg_hit_odds 18.44・payout_per_hit 1,810、負けblockは 8.52・734。利益悪化は高配当ヒット不足で説明。**judgment**: 分析知見として hold。tool: `python3 -m kyotei_predictor.tools.run_exp0053_payout_regime_analysis`。log: experiments/logs/EXP-0053_payout_regime_analysis.md。結果: outputs/selection_verified/exp0053_payout_regime_analysis_results.json。
- **EXP-0054**: Low Payout Regime Filter（d_hi475+switch_dd4000 に EV≥4.50/4.60、odds≥10/12/15、top1_prob≤0.35 を追加、n_w=36）。**結果**: 全 CASE で baseline より profit・ROI 改善。CASE2 最良（profit 17,592）、CASE6 バランス型（profit 8,764、longest_lose 5）。**judgment**: hold。tool: `python3 -m kyotei_predictor.tools.run_exp0054_low_payout_filter`。log: experiments/logs/EXP-0054_low_payout_filter.md。結果: outputs/selection_verified/exp0054_low_payout_filter_results.json。
- **EXP-0055**: Low Payout Filter 頑健性（baseline/CASE2/CASE4/CASE5/CASE6 を n_w=24/30/36 で比較）。**結果**: CASE2 が全期間で profit 1 位だが bet 少・longest_lose 9・block 依存大。CASE6 が 30/36 で 2 位・安定。**judgment**: CASE2 を攻め版、CASE6 を実運用版として 2 本立て採用。tool: `python3 -m kyotei_predictor.tools.run_exp0055_low_payout_filter_robustness`。log: experiments/logs/EXP-0055_low_payout_filter_robustness.md。結果: outputs/selection_verified/exp0055_low_payout_filter_robustness_results.json。
- **EXP-0056**: CASE6 ベース複合条件（CASE6+EV≥4.30/4.40/4.50 を n_w=24/30/36 で比較）。**結果**: CASE6_ev_ge_450 が profit 最良だが bet 297・longest_lose 10。CASE6 単体が bet 533・longest_lose 5 でバランス最良。**judgment**: CASE6 単体を標準採用、CASE6+軽EVフィルタへの置き換えは行わない。tool: `python3 -m kyotei_predictor.tools.run_exp0056_case6_composite_filter`。log: experiments/logs/EXP-0056_case6_composite_filter.md。結果: outputs/selection_verified/exp0056_case6_composite_filter_results.json。
- **EXP-0070**: d_hi475 local search（EV 帯・prob 下限の局所探索、CASE0〜7、n_w=36）。**結果**: CASE2（4.50≤EV<4.75, prob≥0.05）が ROI 11.12%、profit 5,772、max_dd 8,838 で最良。prob_min=0.06 は全 CASE 悪化。**judgment**: adopt（CASE2 を主軸候補）。tool: `python3 -m kyotei_predictor.tools.run_exp0070_d_hi475_local_search --n-windows 36`。log: experiments/logs/EXP-0070_d_hi475_local_search.md。結果: outputs/d_hi475_local_search/exp0070_d_hi475_local_search.json。
- **EXP-0071**: CASE2 robustness check（CASE0/CASE2/CASE3 を n_w=24/30/36 で比較）。**結果**: CASE2 は全 horizon で ROI・max_dd・profit/1k で CASE0 を上回る。total_profit は n_w=30/36 で CASE2 優位、n_w=24 では CASE0(11,744)/CASE3(13,386) の方が高い。**judgment**: 条件付き採用。n_w=36 標準なら CASE2 主軸でよい。tool: `python3 -m kyotei_predictor.tools.run_exp0071_case2_robustness_check`。log: experiments/logs/EXP-0071_case2_robustness_check.md。結果: outputs/case2_robustness/exp0071_case2_robustness.json（gitignoreのため未コミット）。
- **EXP-0075**: probability cap experiment（EXP-0074 で prob≥0.12 が損失源と判明したため、4.50≤EV<4.75, prob≥0.05 に prob 上限を追加）。CASE0（baseline）〜CASE4（prob<0.08）を比較、ref_profit は EXP-0070 同一（4.30≤EV<4.75）、n_w=36。**結果**: 全 cap CASE が baseline を大幅上回る。最良 CASE3（0.05≤prob<0.09）: ROI 187.63%、profit 15,010、max_dd 3,980。**judgment**: adopt。実運用候補を CASE3（prob cap 0.09）に更新。tool: `python3 -m kyotei_predictor.tools.run_exp0075_prob_cap_experiment --n-windows 36`。log: experiments/logs/EXP-0075_prob_cap_experiment.md。結果: outputs/prob_cap/exp0075_prob_cap.json（gitignoreのため未コミット）。
