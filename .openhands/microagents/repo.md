...
- [x] RL学習ループ統合（stable-baselines3/PPOサンプル実装・動作確認）
- [ ] 学習ログ・回収率可視化
...
- サンプル実装（tools/rl_learn_sample.py）でPPOによる学習・推論・モデル保存まで動作確認済み。
...
| RL設計方針   | ✅   | 3連単・損益ベースで決定 |
| 学習ループ   | ✅   | PPO×KyoteiEnvManagerサンプル実装・動作確認OK |
| 状態設計     | ⏳   | 特徴量リスト・前処理検討中 |
...
- [ ] 学習ログ・回収率の可視化（TensorBoard/matplotlib）
- [ ] 学習済みモデルの保存・推論サンプル作成
