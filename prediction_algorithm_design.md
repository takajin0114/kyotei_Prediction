...
### 着順依存性・艇間相関を考慮した3連単確率モデル（拡張方針）

**現状の課題**:
- 現行モデルは「各艇の着順が独立」と仮定しているため、実際のレースで見られる「艇間の相性」や「1着・2着・3着の依存性」を考慮できない。
- 例: 1号艇が1着の場合、2着・3着の艇の確率分布は変化するはずだが、現行は単純な積で計算。

**拡張方針**:
- 着順ごとに「条件付き確率」や「艇間相関」を導入し、より現実的な組み合わせ確率を算出する。
- 例: 1着が決まった後、残りの艇の2着確率を再計算し、さらに3着も同様に条件付きで計算。
- 過去データから「特定の艇同士の着順パターン」や「コースごとの連動傾向」を統計的に抽出し、相関パラメータとして利用。

**実装例イメージ**:
```python
# 1着を決めた後、残りの艇の2着確率を再計算
for first in pit_numbers:
    prob_first = ... # 1着確率
    remaining = [p for p in pit_numbers if p != first]
    for second in remaining:
        prob_second = ... # 2着条件付き確率（firstが1着のとき）
        third = [p for p in remaining if p != second][0]
        prob_third = ... # 3着条件付き確率（first, secondが決まったとき）
        combination_prob = prob_first * prob_second * prob_third
        # ...
```
- 条件付き確率や相関パラメータは、過去レースデータの分析や機械学習で推定可能。
- 将来的には「ベイズネットワーク」や「マルコフモデル」等の応用も検討。

**今後の展望**:
- 独立仮定モデルと条件付き確率モデルの比較・検証
- 相関パラメータの自動学習・最適化
- 実際の的中率・回収率向上への寄与を評価
